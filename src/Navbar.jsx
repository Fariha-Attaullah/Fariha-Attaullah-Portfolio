      ...
      <nav className={`h-36 w-full p-20 flex items-center justify-between navbar text-black top-0 left-0 fixed bg-white ${activeSection !== 'Home' ? 'active' : ''}`}>
        ...
        <ul className='h-16 items-center gap-8 text-lg font-medium transition-all navlist md:flex hidden'>
          {sections.map((section) => (
            <li key={section} onClick={() => handleMenuItemClick(section)} className={`cursor-pointer ${activeSection === section ? 'text-purple-700' : ''}`}>
              {section}
              {activeSection === section && <div className='h-[3px] w-[60%] bg-purple-700 rounded-sm transition-all top-9 move'></div>}
            </li>
          ))}
        </ul>
      </nav>
      <div className={`fixed left-0 top-[120px] h-screen bg-white w-full sidebar transform ${isSidebarOpen ? 'translate-y-0' : '-translate-y-full'} transition-transform ease-in-out`}>
